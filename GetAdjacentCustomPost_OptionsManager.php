...
class GetAdjacentCustomPost_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'get-adjacent-custom-post'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'get-adjacent-custom-post'); ?></h2>
            <table class="form-table"><tbody>
            <tr><td><?php _e('System', 'get-adjacent-custom-post'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'get-adjacent-custom-post'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'get-adjacent-custom-post');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'get-adjacent-custom-post'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'get-adjacent-custom-post');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'get-adjacent-custom-post'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'get-adjacent-custom-post') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'get-adjacent-custom-post');
            case 'false':
                return __('false', 'get-adjacent-custom-post');

            case 'Administrator':
                return __('Administrator', 'get-adjacent-custom-post');
            case 'Editor':
                return __('Editor', 'get-adjacent-custom-post');
            case 'Author':
                return __('Author', 'get-adjacent-custom-post');
            case 'Contributor':
                return __('Contributor', 'get-adjacent-custom-post');
            case 'Subscriber':
                return __('Subscriber', 'get-adjacent-custom-post');
            case 'Anyone':
                return __('Anyone', 'get-adjacent-custom-post');
        }
        return $optionValue;
    }
